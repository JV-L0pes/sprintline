...
| RN-30 colunas do board editáveis (nome, WIP, ordem, categoria de status, adicionar/remover) com ≥1 por categoria, remoção só se vazia e reclassificação que não pode esvaziar categoria | `work/domain/entities.py` (`Board`), `work/application/use_cases.py` | `test_board_columns.py`, `::test_change_column_category_with_coverage_guard` |
