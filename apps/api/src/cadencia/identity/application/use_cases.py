...
class RegisterUser:
    ...
    async def execute(self, *, email: str, name: str, password: str, locale: str) -> User:
        ...
        if await self._users.find_by_email(email.strip().lower()) is not None:
            raise ConflictError("Email jÃ¡ cadastrado", code="EMAIL_ALREADY_REGISTERED")
        user = User.register(
            email=email,
            name=name,
...
class AuthenticateUser:
    ...
    async def execute(self, *, email: str, password: str) -> User:
        ...
        if user is None or not self._hasher.verify(user.password_hash, password):
            raise UnauthorizedError("Credenciais invÃ¡lidas", code="INVALID_CREDENTIALS")
        return user
...
class RotateSession:
    ...
    async def execute(self, raw_refresh: str) -> SessionView:
        ...
        if session is None:
            raise UnauthorizedError("SessÃ£o invÃ¡lida", code="SESSION_NOT_FOUND")
        if session.is_revoked:
            await self._sessions.revoke_family(session.family_id, now)
            raise UnauthorizedError(
                "Reuso de token detectado; sessÃ£o revogada", code="SESSION_REUSE_DETECTED"
            )
        ...
        if user is None:
            raise UnauthorizedError("UsuÃ¡rio nÃ£o encontrado", code="SESSION_USER_MISSING")
...
class CreateWorkspace:
    ...
    async def execute(self, *, user: User, name: str, timezone: str) -> WorkspaceView:
        ...
        if await self._workspaces.find_by_slug(workspace.slug) is not None:
            raise ConflictError("JÃ¡ existe um workspace com este nome", code="WORKSPACE_EXISTS")
        ...
        *,
        ...
        if not role_at_least(actor_role, role):
            raise ForbiddenError(
                "NÃ£o e possÃ­vel convidar com papel superior ao seu", code="ROLE_ESCALATION"
            )
        ...
        if existing_user is not None:
            ...
            if membership is not None:
                raise ConflictError("UsuÃ¡rio jÃ¡ Ã© membro do workspace", code="ALREADY_MEMBER")
        now = self._clock.now()
...
class AcceptInvite:
    ...
    async def execute(self, *, token: str, user: User) -> WorkspaceView:
        ...
        if invite is None:
            raise NotFoundError("Convite nÃ£o encontrado", code="INVITE_NOT_FOUND")
        ...
        if workspace is None:
            raise NotFoundError("Workspace nÃ£o encontrado", code="WORKSPACE_NOT_FOUND")
        if existing is not None:
            # Idempotente: jÃ¡ Ã© membro (ex.: refresh da pagina de convite).
...
def _ensure_can_manage(*, actor_role: Role, target_role: Role) -> None:
    ...
    if ROLE_RANK[target_role] >= ROLE_RANK[actor_role]:
        raise ForbiddenError(
            "Seu papel nÃ£o permite gerenciar este membro", code="CANNOT_MANAGE_MEMBER"
        )
...
def _ensure_role_allowed(*, actor_role: Role, new_role: Role) -> None:
    if not role_at_least(actor_role, new_role):
        raise ForbiddenError(
            "NÃ£o e possÃ­vel atribuir papel superior ao seu", code="ROLE_ESCALATION"
        )
    ...
    async def execute(
        ...
        *,
        ...
        if target_user_id == actor_id:
            raise ConflictError(
                "VocÃª nÃ£o pode alterar seu prÃ³prio papel", code="CANNOT_MANAGE_SELF"
            )
        target = await self._memberships.get(workspace_id, target_user_id)
        if target is None:
            raise NotFoundError("Membro nÃ£o encontrado", code="MEMBER_NOT_FOUND")
        ...
        if user is None:
            raise NotFoundError("UsuÃ¡rio nÃ£o encontrado", code="USER_NOT_FOUND")
...
class RemoveMember:
    ...
    async def execute(
        ...
        *,
        ...
        if target_user_id == actor_id:
            raise ConflictError("VocÃª nÃ£o pode remover a si mesmo", code="CANNOT_MANAGE_SELF")
        target = await self._memberships.get(workspace_id, target_user_id)
        if target is None:
            raise NotFoundError("Membro nÃ£o encontrado", code="MEMBER_NOT_FOUND")
        _ensure_can_manage(actor_role=actor_role, target_role=target.role)
...
class ChangeOwnPassword:
    ...
    async def execute(self, *, user: User, current_password: str, new_password: str) -> None:
        ...
        user.set_password_hash(self._hasher.hash(new_password))
        await self._users.save(user)
        # Toda a famÃ­lia de sessÃµes e revogada: re-login em todos os dispositivos.
        await self._sessions.revoke_all_for_user(user.id, self._clock.now())
